...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		...
		// set new tableSize to 2x
		tableSize *= 2;
		numEntries = 0;
		table = new LinkedList[tableSize];

		// iterate over the oldTable array
		// find index that have non-null elements and add them to the new list
		for (int i = 0; i < oldTable.length; i++) {
			// get the linked-list
			var mList = oldTable[i];

			// add the list in the newly created list
			while (mList != null && !mList.isEmpty()) {
				var node = (Entry) mList.poll();
